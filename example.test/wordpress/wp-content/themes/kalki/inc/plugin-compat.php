...
add_action( 'after_setup_theme', 'kalki_setup_woocommerce' );
/**
 * Set up WooCommerce
 *
 */
function kalki_setup_woocommerce() {
	...
	add_action( 'woocommerce_sidebar', 'kalki_construct_sidebars' );
}

if ( ! function_exists( 'kalki_woocommerce_start' ) ) {
	add_action( 'woocommerce_before_main_content', 'kalki_woocommerce_start', 10 );
	/**
	 * Add WooCommerce starting wrappers
	 *
	 */
	function kalki_woocommerce_start() { ?>
...
if ( ! function_exists( 'kalki_woocommerce_end' ) ) {
	add_action( 'woocommerce_after_main_content', 'kalki_woocommerce_end', 10 );
	/**
	 * Add WooCommerce ending wrappers
	 *
	 */
	function kalki_woocommerce_end() { ?>
						</div><!-- .entry-content -->
						<?php
						/**
...
if ( ! function_exists( 'kalki_woocommerce_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'kalki_woocommerce_css', 100 );
	/**
	 * Add WooCommerce CSS
	 *
	 */
	function kalki_woocommerce_css() {
		if ( ! class_exists( 'WooCommerce' ) ) {
			return;
		}

		$mobile = apply_filters( 'kalki_mobile_media_query', '(max-width:768px)' );
...
if ( ! function_exists( 'kalki_bbpress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'kalki_bbpress_css', 100 );
	/**
	 * Add bbPress CSS
	 *
	 */
	function kalki_bbpress_css() {
...
if ( ! function_exists( 'kalki_buddypress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'kalki_buddypress_css', 100 );
	/**
	 * Add BuddyPress CSS
	 *
	 */
	function kalki_buddypress_css() {
...
if ( ! function_exists( 'kalki_beaver_builder_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'kalki_beaver_builder_css', 100 );
	/**
	 ...
	 *
	 */
	function kalki_beaver_builder_css() {
