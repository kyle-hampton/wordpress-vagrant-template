...
if ( ! function_exists( 'kalki_enqueue_google_fonts' ) ) {
	add_action( 'wp_enqueue_scripts', 'kalki_enqueue_google_fonts', 0 );
	/**
	 * Add Google Fonts to wp_head if needed.
	 *
	 */
	function kalki_enqueue_google_fonts() {
		...
		$not_google = str_replace( ' ', '+', kalki_typography_default_fonts() );
		...
		if ( ! empty( $font_settings ) ) {
			...
			foreach ( $font_settings as $key ) {
				...
				if ( ! isset( $kalki_settings[$key] ) ) {
					continue;
				}

				// If our value is still using the old format, fix it
				if ( strpos( $kalki_settings[$key], ':' ) !== false ) {
					$kalki_settings[$key] = current( explode( ':', $kalki_settings[$key] ) );
				}

				// Replace the spaces in the names with a plus
				$value = str_replace( ' ', '+', $kalki_settings[$key] );

				// Grab the variants using the plain name
				$variants = kalki_get_google_font_variants( $kalki_settings[$key], $key );
		...
		$google_fonts = apply_filters( 'kalki_typography_google_fonts', $google_fonts );

		// Get the subset
		$subset = apply_filters( 'kalki_fonts_subset','' );
		...
		$default_fonts = KALKI_DEFAULT_FONTS;

		// Create our URL using the arguments
		$fonts_url = add_query_arg( $font_args, '//fonts.googleapis.com/css' );

		// Enqueue our fonts
		wp_enqueue_style('kalki-default-fonts', $default_fonts, array(), null, 'all' );
		if ( $google_fonts ) {
			wp_enqueue_style('kalki-fonts', $fonts_url, array(), null, 'all' );
		}
	}
}

if ( ! function_exists( 'kalki_default_fonts_customize_register' ) ) {
	add_action( 'customize_register', 'kalki_default_fonts_customize_register' );
	/**
	 * Build our Typography options
	 *
	 */
	function kalki_default_fonts_customize_register( $wp_customize ) {
		if ( function_exists( 'kalki_fonts_customize_register' ) ) {
			// Bail if Kalki Premium is active
			return;
		}
		...
		$defaults = kalki_get_default_fonts();

		if ( method_exists( $wp_customize,'register_control_type' ) ) {
			$wp_customize->register_control_type( 'Kalki_Typography_Customize_Control' );
		...
		$wp_customize->add_setting(
			...
			array(
				...
				'sanitize_callback' => 'kalki_sanitize_variants'
			)
		);

		$wp_customize->add_setting(
			'kalki_settings[body_font_weight]',
			array(
		...
		$wp_customize->add_setting(
			'kalki_settings[body_font_transform]',
			array(
		...
		$wp_customize->add_control(
			new Kalki_Typography_Customize_Control(
				$wp_customize,
				'body_typography',
				array(
					'section' => 'font_section',
					'priority' => 1,
					'label' => __( 'Text Font', 'kalki' ),
					'settings' => array(
						'family' => 'kalki_settings[font_body]',
						'variant' => 'font_body_variants',
						'category' => 'font_body_category',
						'weight' => 'kalki_settings[body_font_weight]',
						'transform' => 'kalki_settings[body_font_transform]',
					),
				)
			)
		);
		...
		$wp_customize->add_control(
			new Kalki_Range_Slider_Control(
				...
				array(
					...
					'description' => __( 'Font size', 'kalki' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'kalki_settings[body_font_size]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 6,
		...
		$wp_customize->add_setting(
			'kalki_settings[body_line_height]',
			array(
				'default' => $defaults['body_line_height'],
				'type' => 'option',
				'sanitize_callback' => 'kalki_sanitize_decimal_integer',
				'transport' => 'postMessage'
			)
		);
		...
		$wp_customize->add_control(
			new Kalki_Range_Slider_Control(
				...
				array(
					'type' => 'kalki-range-slider',
					'description' => __( 'Line height', 'kalki' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'kalki_settings[body_line_height]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 1,
		...
		$wp_customize->add_setting(
			'kalki_settings[paragraph_margin]',
			array(
		...
		$wp_customize->add_control(
			new Kalki_Range_Slider_Control(
				...
				array(
					'type' => 'kalki-range-slider',
					'description' => __( 'Paragraph margin', 'kalki' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'kalki_settings[paragraph_margin]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 0,
...
if ( ! function_exists( 'kalki_get_all_google_fonts' ) ) {
	/**
	 ...
	 */
	function kalki_get_all_google_fonts( $amount = 'all' ) {
		...
		if ( apply_filters( 'kalki_alphabetize_google_fonts', true ) ) {
			asort( $fonts );
		}

		// Filter to allow us to modify the fonts array
		return apply_filters( 'kalki_google_fonts_array', $fonts );
	}
}

if ( ! function_exists( 'kalki_get_all_google_fonts_ajax' ) ) {
	add_action( 'wp_ajax_kalki_get_all_google_fonts_ajax', 'kalki_get_all_google_fonts_ajax' );
	/**
	 * Return an array of all of our Google Fonts.
	 *
	 *
	 */
	function kalki_get_all_google_fonts_ajax() {
		// Bail if the nonce doesn't check out
		if ( ! isset( $_POST[ 'kalki_customize_nonce' ] ) || ! wp_verify_nonce( sanitize_key( $_POST[ 'kalki_customize_nonce' ] ), 'kalki_customize_nonce' ) ) {
			wp_die();
		}

		// Do another nonce check
		check_ajax_referer( 'kalki_customize_nonce', 'kalki_customize_nonce' );
		...
		$fonts = kalki_get_all_google_fonts();
...
if ( ! function_exists( 'kalki_get_google_font_variants' ) ) {
	/**
	 * Wrapper function to find variants for chosen Google Fonts
	 * Example: kalki_get_google_font_variation( 'Open Sans' )
	 *
	 */
	function kalki_get_google_font_variants( $font, $key = '' ) {
		// Don't need variants if we're using a system font
		if ( in_array( $font, kalki_typography_default_fonts() ) ) {
			return;
		}
		...
		if ( $variants ) {
			...
			return implode(',', apply_filters( 'kalki_typography_variants', $output ) );
		}
	}
}

if ( ! function_exists( 'kalki_get_google_font_category' ) ) {
	/**
	 * Wrapper function to find the category for chosen Google Font
	 * Example: kalki_get_google_font_category( 'Open Sans' )
	 *
	 *
	 *
	 ...
	 */
	function kalki_get_google_font_category( $font, $key = '' ) {
...
if ( ! function_exists( 'kalki_get_font_family_css' ) ) {
	/**
	 ...
	 */
	function kalki_get_font_family_css( $font, $settings, $default ) {
		$kalki_settings = wp_parse_args(
		...
		$no_quotes = array(
			...
			apply_filters( 'kalki_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' )
		);

		// Get our font
		$font_family = $kalki_settings[ $font ];

		if ( 'System Stack' == $font_family ) {
			$font_family = apply_filters( 'kalki_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' );
		}
		...
		if ( in_array( $font_family, $no_quotes ) ) {
			...
			$wrapper_start = '"';
			$wrapper_end = '"' . kalki_get_google_font_category( $font_family, $font );
		}
...
if ( ! function_exists( 'kalki_add_to_font_customizer_list' ) ) {
	add_filter( 'kalki_typography_customize_list', 'kalki_add_to_font_customizer_list' );
	/**
	 ...
	 *
	 */
	function kalki_add_to_font_customizer_list( $fonts ) {
		...
		$all_fonts = kalki_get_all_google_fonts();
		$select_fonts = kalki_get_all_google_fonts( apply_filters( 'kalki_number_of_fonts', 200 ) );

		foreach ( $font_settings as $setting ) {
			// If we don't have a setting, keep going
			if ( ! isset( $kalki_settings[ $setting ] ) ) {
				continue;
			}

			$id = strtolower( str_replace( ' ', '_', $kalki_settings[ $setting ] ) );

			if ( array_key_exists( $id, $select_fonts ) || in_array( $kalki_settings[ $setting ], kalki_typography_default_fonts() ) ) {
				continue;
			}

			$fonts[ strtolower( str_replace( ' ', '_', $kalki_settings[ $setting ] ) ) ] = array(
				'name' => $kalki_settings[ $setting ],
		...
		if ( function_exists( 'kalki_secondary_nav_get_defaults' ) ) {
			$secondary_nav_settings = wp_parse_args(
				get_option( 'kalki_secondary_nav_settings', array() ),
				kalki_secondary_nav_get_defaults()
			);

			$secondary_nav_id = strtolower( str_replace( ' ', '_', $secondary_nav_settings[ 'font_secondary_navigation' ] ) );

			if ( ! array_key_exists( $secondary_nav_id, $select_fonts ) && ! in_array( $secondary_nav_settings[ 'font_secondary_navigation' ], kalki_typography_default_fonts() ) ) {
