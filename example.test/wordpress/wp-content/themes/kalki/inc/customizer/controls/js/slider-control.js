wp.customize.controlConstructor['kalki-range-slider'] = wp.customize.Control.extend({
	...
	ready: function() {
		...
		var control = this,
		    ...
		    inputDefault,
		    changeAction,
			controlClass = '.customize-control-kalki-range-slider',
			footerActions = jQuery( '#customize-footer-actions' );
		
		// Set up the sliders
		jQuery( '.kalki-slider' ).each( function() {
		...
		jQuery( controlClass + ' .kalki_range_value input[type=number]' ).on( 'input', function() {
			value = jQuery( this ).attr( 'value' );
			if ( '' == value ) {
				value = -1;
			}
			jQuery( this ).closest( 'label' ).find( '.kalki-slider' ).slider( 'value', parseFloat(value)).change();
		});

		// Handle the reset button
		jQuery( controlClass + ' .kalki-reset' ).on( 'click', function() {
			var icon = jQuery( this ),
				visible_area = icon.closest( '.kalki-range-title-area' ).next( '.kalki-range-slider-areas' ).children( 'label:visible' ),
				input = visible_area.find( 'input[type=number]' ),
				slider_value = visible_area.find( '.kalki-slider' ),
				visual_value = visible_area.find( '.kalki_range_value' ),
		...
		jQuery( controlClass + ' .kalki-range-slider-control' ).each( function() {
			var _this = jQuery( this );
			_this.find( '.kalki-device-controls' ).children( 'span:first-child' ).addClass( 'selected' );
			_this.find( '.range-option-area:first-child' ).show();
		});
		
		// Do stuff when device icons are clicked
		jQuery( controlClass + ' .kalki-device-controls > span' ).on( 'click', function( event ) {
			var device = jQuery( this ).data( 'option' );
			
			jQuery( controlClass + ' .kalki-device-controls span' ).each( function() {
				var _this = jQuery( this );
			...
			jQuery( controlClass + ' .kalki-range-slider-areas label' ).each( function() {
				var _this = jQuery( this );
				if ( device == _this.attr( 'data-option' ) ) {
					_this.show();
					_this.siblings().hide();
