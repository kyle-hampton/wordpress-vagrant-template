...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Kalki_Typography_Customize_Control' ) ) {
	/**
	 * Create the typography elements control.
	 *
	 */
	class Kalki_Typography_Customize_Control extends WP_Customize_Control {
		public $type = 'kalki-customizer-typography';

		public function enqueue() {
			wp_enqueue_script( 'kalki-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/selectWoo.min.js', array( 'customize-controls', 'jquery' ), KALKI_VERSION, true );
			wp_enqueue_style( 'kalki-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/selectWoo.min.css', array(), KALKI_VERSION );

			wp_enqueue_script( 'kalki-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/typography-customizer.js', array( 'customize-controls', 'kalki-typography-selectWoo' ), KALKI_VERSION, true );
			wp_enqueue_style( 'kalki-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/typography-customizer.css', array(), KALKI_VERSION );
		}

		public function to_json() {
			parent::to_json();

			$number_of_fonts = apply_filters( 'kalki_number_of_fonts', 200 );
			$this->json[ 'default_fonts_title'] = __( 'System fonts', 'kalki' );
			$this->json[ 'google_fonts_title'] = __( 'Google fonts', 'kalki' );
			$this->json[ 'google_fonts' ] = apply_filters( 'kalki_typography_customize_list', kalki_get_all_google_fonts( $number_of_fonts ) );
			$this->json[ 'default_fonts' ] = kalki_typography_default_fonts();
			$this->json[ 'family_title' ] = esc_html__( 'Font family', 'kalki' );
			$this->json[ 'weight_title' ] = esc_html__( 'Font weight', 'kalki' );
			$this->json[ 'transform_title' ] = esc_html__( 'Text transform', 'kalki' );
			$this->json[ 'category_title' ] = '';
			$this->json[ 'variant_title' ] = esc_html__( 'Variants', 'kalki' );
		...
		public function content_template() {
			...
			<# if ( 'undefined' !== typeof ( data.variant ) ) { #>
				...
				#>
				<div id={{{ data.variant.id }}}" class="kalki-font-variant" data-saved-value="{{ data.variant.value }}">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.category ) ) { #>
				<div class="kalki-font-category">
					<label>
							<input name="{{{ data.category.id }}}" type="hidden" {{{ data.category.link }}} value="{{{ data.category.value }}}" class="kalki-hidden-input" />
			...
			<# if ( 'undefined' !== typeof ( data.transform ) ) { #>
				<div class="kalki-font-transform">
					<label>
