...
if ( ! function_exists( 'kalki_get_setting' ) ) {
	/**
	 * A wrapper function to get our settings.
	 *
	 */
	function kalki_get_setting( $setting ) {
		$kalki_settings = wp_parse_args(
			get_option( 'kalki_settings', array() ),
			kalki_get_defaults()
		);

		return $kalki_settings[ $setting ];
	}
}

if ( ! function_exists( 'kalki_get_layout' ) ) {
	/**
	...
	function kalki_get_layout() {
		...
		$layout = $kalki_settings['layout_setting'];

		// Get the individual page/post sidebar metabox value
		$layout_meta = ( isset( $post ) ) ? get_post_meta( $post->ID, '_kalki-sidebar-layout-meta', true ) : '';
		...
		if ( is_single() && ! $buddypress ) {
			$layout = null;
			$layout = $kalki_settings['single_layout_setting'];
		}
		...
		if ( is_home() || is_archive() || is_search() || is_tax() ) {
			$layout = null;
			$layout = $kalki_settings['blog_layout_setting'];
		}

		// Finally, return the layout
		return apply_filters( 'kalki_sidebar_layout', $layout );
	}
}

if ( ! function_exists( 'kalki_get_footer_widgets' ) ) {
	/**
	 ...
	 */
	function kalki_get_footer_widgets() {
		// Get current post
		global $post;
		...
		$widgets = $kalki_settings['footer_widget_setting'];

		// Get the individual footer widget metabox value
		$widgets_meta = ( isset( $post ) ) ? get_post_meta( $post->ID, '_kalki-footer-widget-meta', true ) : '';
		...
		// Finally, return the layout
		return apply_filters( 'kalki_footer_widgets', $widgets );
	}
}

if ( ! function_exists( 'kalki_show_excerpt' ) ) {
	/**
	 * Figure out if we should show the blog excerpts or full posts
	 *
	 */
	function kalki_show_excerpt() {
		// Get current post
		global $post;
		...
		$more_tag = apply_filters( 'kalki_more_tag', strpos( $post->post_content, '<!--more-->' ) );

		// Check the post format
		$format = ( false !== get_post_format() ) ? get_post_format() : 'standard';

		// Get the excerpt setting from the Customizer
		$show_excerpt = ( 'excerpt' == $kalki_settings['post_content'] ) ? true : false;
		...
		// Return our value
		return apply_filters( 'kalki_show_excerpt', $show_excerpt );
	}
}

if ( ! function_exists( 'kalki_show_title' ) ) {
	/**
	 ...
	 */
	function kalki_show_title() {
		return apply_filters( 'kalki_show_title', true );
	}
}

if ( ! function_exists( 'kalki_padding_css' ) ) {
	/**
	 ...
	 */
	function kalki_padding_css( $top, $right, $bottom, $left ) {
...
if ( ! function_exists( 'kalki_get_link_url' ) ) {
	/**
	 * Return the post URL.
	 *
...
if ( ! function_exists( 'kalki_get_navigation_location' ) ) {
	/**
	 ...
	 */
	function kalki_get_navigation_location() {
		return apply_filters( 'kalki_navigation_location', kalki_get_setting( 'nav_position_setting' ) );
	}
}
